...
(function () {
  ...
  function injectSEO(course) {
    ...
    var schema = {
      ...
      'offers': {
        '@type':         'Offer',
        'price':         String(Math.round(course.price * ((U.t(META.currencyRate)) || 1))),
        'priceCurrency': U.getLang() === 'ar' ? 'EGP' : 'USD',
  ...
  function buildBreadcrumb(course) {
    ...
    var homeLink = U.el('a', { href: U.sanitizeUrl(SP.buildHomeUrl()) });
    homeLink.appendChild(U.el('i', { className: 'bi bi-house-door-fill', aria: { hidden: 'true' } }));
    homeLink.appendChild(document.createTextNode(' ' + U.t(META.breadcrumbHome)));
  ...
  function buildHeader(course) {
    ...
    var backLink = U.el('a', { className: 'back-link', href: U.sanitizeUrl(SP.buildCatalogUrl()) }, [
      U.el('i', { className: backArrow, aria: { hidden: 'true' } }),
      U.t(META.backToCourses)
    ]);

    var langSlot = U.el('div', { className: 'details-header-lang' });
    langSlot.appendChild(SP.buildLangSwitcher());

    var topRow = U.el('div', { className: 'details-header-top' }, [
      backLink,
      langSlot
    ]);
  ...
  /* ============================================================
     VIDEO PREVIEW SYSTEM — Facade Pattern
     ============================================================ */

  /**
   * Detects video provider from URL.
   * Returns { provider: 'youtube'|'drive'|'unknown', id: string }
   */
  function _detectProvider(url) {
    if (!url || typeof url !== 'string') return { provider: 'unknown', id: '' };
    var ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtube\.com\/embed\/|youtu\.be\/|youtube-nocookie\.com\/embed\/)([a-zA-Z0-9_-]{11})/);
  ...
  /**
   * Builds the embed URL for the provider.
   */
  function _buildEmbedUrl(info) {
    if (info.provider === 'youtube') {
      return 'https://www.youtube-nocookie.com/embed/' + info.id + '?autoplay=1&rel=0';
    }
   ...
   */
  function _buildThumbnailUrl(info) {
    if (info.provider === 'youtube') {
  ...
  /**
   * Returns provider display label.
   */
  function _providerLabel(info) {
    if (info.provider === 'youtube') return 'YouTube';
    if (info.provider === 'drive') return 'Google Drive';
    return '';
  }
   ...
   */
  function _buildFullscreenOverlay(lesson, providerInfo) {
    var embedUrl = _buildEmbedUrl(providerInfo);
    ...
    var header = U.el('div', { className: 'preview-overlay-header' }, [
      U.el('p', { className: 'preview-overlay-title', textContent: lesson.title }),
      closeBtn
    ]);
    ...
    var overlay = U.el('div', {
      ...
      role:      'dialog',
      aria:      { label: lesson.title, modal: 'true' }
    }, [
      backdrop,
      content
    ]);
   ...
   */
  function _openFullscreen(lesson, providerInfo, triggerEl) {
    if (_activePreview.overlayEl) _closeFullscreen();

    var overlay = _buildFullscreenOverlay(lesson, providerInfo);
    _activePreview.overlayEl = overlay;

    document.body.appendChild(overlay.element);

    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        overlay.element.classList.add('preview-overlay--open');
      });
    });
    ...
    overlay.backdrop.addEventListener('click', function () {
      _closeFullscreen();
      if (triggerEl) triggerEl.focus();
    });

    var _fsKeyHandler = function (e) {
      if (e.key === 'Escape') {
        e.preventDefault();
        _closeFullscreen();
        if (triggerEl) triggerEl.focus();
      ...
      if (e.key === 'Tab') {
        var focusable = overlay.element.querySelectorAll('button, iframe, [tabindex]:not([tabindex="-1"])');
    ...
    overlay.element._keyHandler = _fsKeyHandler;
  }
  ...
  function _closeFullscreen() {
    if (!_activePreview.overlayEl) return;
    var overlay = _activePreview.overlayEl;
    ...
    el.classList.remove('preview-overlay--open');
  ...
  function initChatEvents(courseId) {
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && chatState.isOpen) { _toggleChat(); return; }
      if (e.key === 'Tab' && chatState.isOpen) {
        var widget = U.qs('#chat-widget');
        if (!widget) return;
        var focusable = widget.querySelectorAll('button:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');
        if (focusable.length === 0) return;
        var first = focusable[0];
        var last  = focusable[focusable.length - 1];
        if (e.shiftKey) {
  ...
  function init() {
    ...
    requestAnimationFrame(function () {
      ...
      if (titleEl) titleEl.scrollIntoView({ behavior: 'auto', block: 'start' });
    });
