...
window.SharedPage = (function () {
  ...
  function formatOriginalPrice(originalPrice) {
    if (!originalPrice || originalPrice <= 0) return '';
    var symbol = U.t(META.currencyLabel);
    var rate   = U.t(META.currencyRate) || 1;
    var converted = Math.round(originalPrice * rate);
    return U.formatNumberByLang(converted) + ' ' + symbol;
  }
  ...
  function formatDiscount(price, originalPrice) {
    ...
    var symbol = U.t(META.currencyLabel);
    var rate   = U.t(META.currencyRate) || 1;
    var saved  = Math.round((originalPrice - price) * rate);
    var pct    = Math.round(((originalPrice - price) / originalPrice) * 100);
    var template = U.t(META.discountLabel);
    return template
      .replace('{pct}', U.formatNumberByLang(pct))
      .replace('{amount}', U.formatNumberByLang(saved) + ' ' + symbol);
  }
  ...
  function buildFooter() {
    ...
    if (mapSlot && DATA.GOOGLE_MAPS_EMBED) {
      ...
      var iframe = U.el('iframe', {
        ...
        loading:   'lazy',
        aria:      { hidden: 'true' },
        title:     mapTitle + ' — Ai8V, Tanta, Egypt',
        tabindex:  '-1'
      });
